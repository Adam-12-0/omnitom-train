...
- The failed format-control Double-Agent evaluation `787528` exposed a second issue: its descriptive adapter directory was parsed as base model `double`. The model resolver now explicitly maps that schema-only adapter to Qwen3-14B. Corrected full evaluation `787751` has been submitted against the live open-weight attacker/judge services.
